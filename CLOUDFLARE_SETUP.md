# Cloudflare Architecture — Randevou
...
wrangler d1 execute randevou-db --local --file=migrations/0001_rezavu_core.sql
wrangler d1 execute randevou-db --local --file=migrations/0002_booking_rules.sql
```
...
wrangler pages secret put TWILIO_WHATSAPP_FROM --project-name=randevou    # e.g. whatsapp:+14155238886
...
```bash
wrangler pages dev --d1 DB=randevou-db --kv KV=randevou-sessions
```
...
```bash
wrangler r2 bucket create randevou-assets
```
